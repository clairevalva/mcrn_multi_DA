...
yy = get_case_data_city.fetch()[obs_remove:]
...
# Initial conditions
mu0 = np.array([168000 - yy[1, 0], .05 * yy[1, 0], .95 * yy[1, 0], .20]) # initial conditions SEQIRD and C
...
axs[1].legend()
plt.xlabel("Time (t)")
plt.savefig("assimilated_data/SIRC_assimilation_city.png")

np.save("assimilated_data/SIRC_beta=1_city.npy", xxhat)
